...
def abstract_id(paper_id):
    ...
    if req.status_code != 200:
        print(
            f"Failed to retrieve abstract for paper {paper_id}.\
    ...
    try:
        ...
        print(
            f"Failed to decode JSON for paper {paper_id}.\
        Response: {req.text}"
        )
        return None
...
def openalex_search(query, start_year, end_year):
    ...
    sorted_results = sorted(
        all_results, key=lambda x: x.get("cited_by_count", 0), reverse=True
    )
